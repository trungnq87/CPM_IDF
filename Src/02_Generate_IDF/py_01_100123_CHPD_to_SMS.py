# ============================================================= #
#
# Step 1: Aggregation of C-HPD series to Seasonal Maximum Series 
...
def get_argument() :
    ...
    out1=config['input']
    out2=config['output']
    out3=config['startyear']
    out4=config['endyear']

    out5=config['firsthour']
    out6=config['interval']
    out7=config['numberduration']

    out8=config['stationlist']

    out9=config['skiprow']
    out10=config['numberofrow']

    # 294 : oh, should it be an array ?
...
def aggregation(df,staname,fh,inter,nduration) :
    ...
    for i in np.arange(nduration) :
        ...
        df_max=df_max.rename(columns={str(staname):str(j)+'h'})
...
# Get arguments
indir, outdir, startyear, endyear, firsthour, interval, ndura, stalist, skro, nrow = get_argument()
