...
#
# ----------------------------------------------------------------------- #
# Step 3 : Fit C-HPD to GEV distribution
# ----------------------------------------------------------------------- #
#
# Supported Python script
r03="R_03_100223_GEV_fit.Rscript"
currentdir=`pwd`
...
cd $tmpdir
time Rscript --vanilla r_${c}.Rscript -i ${in03}${i}_${s}.csv -g ${out03}/gev_${i}_${s}.out -o ${out03}${i}_${s}.csv -a $alpha -t $cirange -b $nboot -f $frequency -d $minduration
EOF
        #
        sbatch s_${c}.script
        #
