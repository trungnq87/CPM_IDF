...
#
# ----------------------------------------------------------------------- #
# Step 5 : Fit WRF to GEV distribution
# ----------------------------------------------------------------------- #
#
# Supported Python script
r05="R_03_100223_GEV_fit.Rscript" # Same as r03 :)
currentdir=`pwd`
...
cd $tmpdir
time Rscript --vanilla r_${c}.Rscript -i ${in05a}${i}_${s}.csv -g ${out05a}/gev_${i}_${s}.out -o ${out05a}${i}_${s}.csv -a $alpha -t $cirange -b $nboot -f $frequency -d $minduration
EOF
        #
        #sbatch s_${c}.script
        #
...
cd $tmpdir
time Rscript --vanilla r_${c}.Rscript -i ${in05b}${i}_${s}.csv -g ${out05b}/gev_${i}_${s}.out -o ${out05b}${i}_${s}.csv -a $alpha -t $cirange -b $nboot -f $frequency -d $minduration
EOF
        #
        #sbatch s_${c}.script
        #
