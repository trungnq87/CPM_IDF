...
def aggregation(df,staname,fh,inter,nduration) :
    ...
    for i in np.arange(nduration) :
        j=fh+inter*i
        # Rounding to avoid numeric floating
        #df_round=df.round(decimals=3) # 3 is too much but then
        # Running sum
        #df_r=df_round.rolling(int(j)).sum() # Think about min_periods OR fillna here !
        # Or rounding here
        df_r=np.around(df.rolling(int(j)).sum(),decimals=3)
        # Get annual maximum
